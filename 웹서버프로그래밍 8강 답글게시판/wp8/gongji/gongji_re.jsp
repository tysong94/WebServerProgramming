...
<body>
    ...
    // 댓글순서 계산
    int parentRelevel = Integer.parseInt(request.getParameter("relevel"));  // 부모 댓글레벨 가져오기
    int parentRecnt = Integer.parseInt(request.getParameter("recnt"));      // 부모 댓글순서 가져오기
    sql = "select min(recnt) from gongji where rootid = "+rootid+" and relevel <= "+parentRelevel+" and recnt > "+parentRecnt+";";
    rs = stmt.executeQuery(sql);
    while(rs.next()) { 
        recnt = rs.getInt(1); 
        if(recnt == 0) {
            sql = "select max(recnt) + 1 from gongji where rootid = "+rootid+";";
            rs = stmt.executeQuery(sql);
            while(rs.next()) { 
                recnt = rs.getInt(1); 
            }
    ...
    <!-- 내용 -->
    <form name='f' method='post' action='gongji_reinsert.jsp' enctype="Multipart/form-data">
    <table border=1 width=700>
        ...
        <tr>
            ...
            <td width=600><input type='text' name='date' style='width:600;' value='<%=date%>' readonly></td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td width=600>
                <input name='file1' type='file' onchange="CheckUploadFileSize(this);"/>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td width=600><input type=text name='relevel' style='width:600;' value='<%=relevel%>' readonly></td>
        </tr>
        <tr>
            <td width=100 align=center>댓글순서</td>
            <td width=600><input type=text name='recnt' style='width:600;' value='<%=recnt%>' readonly></td>
        </tr>  
    </table>
